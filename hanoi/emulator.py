import time

# Import and initialize the pygame library
import pygame

from enum import Enum
from timeit import default_timer as timer

from pygame.locals import *
from GameMachine import GameMachine
from BitNumber import *

X_LEDS, Y_LEDS = 32, 8
X_BUFFER, Y_BUFFER = 128, 128
LED_SIZE = 32

class Colors(Enum):
    BLACK = (0, 0, 0)
    WHITE = (255, 255, 255)
    BLUE = (0, 0, 255)
    GREEN = (0 ,255, 0)
    RED = (255, 0, 0)

    PLAYER = GREEN
    TOWER = BLUE
    ENEMY = RED


class Emulator(object):
    def __init__(self):
        self.state = GameMachine()
        self.screen = None

    def init_screen(self):
        self.screen = pygame.display.set_mode([
            2 * X_BUFFER + X_LEDS * LED_SIZE,
            2 * Y_BUFFER + Y_LEDS * LED_SIZE
        ])

        return self.screen

    def fill_leds(self):
        num_pixels = X_LEDS * Y_LEDS
        tower_x_positions, tower_y_positions = [], []
        enemy_positions = []

        player_pos: UBitNumber = self.state.player_pos
        player_x, player_y = self.extract_position(player_pos)

        for tower_no in range(3):
            tower = self.state.tower_positions[tower_no]
            tower_x, tower_y = self.extract_position(tower)
            # print(tower_no, tower_x, tower_y, tower)
            tower_x_positions.append(tower_x)
            tower_y_positions.append(tower_y)

        for enemy_no in range(8):
            enemy_pos = self.state.enemy_positions[enemy_no]
            enemy_dir = self.state.enemy_directions[enemy_no]
            # enemies that aren't moving are considered invisible
            if enemy_dir == 0:
                continue

            enemy_x, enemy_y = self.extract_position(enemy_pos)
            enemy_positions.append((enemy_x, enemy_y))

        for k in range(num_pixels):
            x, y, = k % X_LEDS, k // X_LEDS
            led_color = Colors.BLACK
            coords = (x, y)

            # print(f'{player_x, player_y}')
            if (player_x == x) and (player_y == y):
                led_color = Colors.PLAYER
            elif coords in enemy_positions:
                led_color = Colors.ENEMY
            elif x in tower_x_positions:
                index = tower_x_positions.index(x)
                tower_y = tower_y_positions[index]
                if tower_y < y:
                    led_color = Colors.TOWER

            led_color = led_color.value
            pygame.draw.rect(self.screen, led_color, rect=(
                X_BUFFER + x * LED_SIZE, Y_BUFFER + y * LED_SIZE,
                LED_SIZE - 1, LED_SIZE - 1
            ))

    @staticmethod
    def extract_position(bit_position: UBitNumber):
        # decode the 8 bit position register into x and y
        # integer coordinates respectively
        x, y = int(bit_position[4:0]), int(bit_position[7:5])
        return x, y

    def run(self):
        pygame.init()
        self.init_screen()
        start_time = timer()
        running = True
        frames = 0

        PMOVE = UBitNumber(0, num_bits=4)
        """
        io_button is simply the 5 push buttons. io_button[0] is up,
        io_button[1] is center, io_button[2] is down, io_button[3]
        is left, and io_button[4] is right.
        """

        while running:
            PMOVE.enable_edit()
            PMOVE.zero_all_bits()

            # Did the user click the window close button?
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K.UP:
                        PMOVE[0] = 1
                    elif event.key == pygame.K.DOWN:
                        PMOVE[1] = 1
                    elif event.key == pygame.K_LEFT:
                        PMOVE[2] = 1
                    elif event.key == pygame.K_RIGHT:
                        PMOVE[3] = 1

            # Fill the background with gray
            self.screen.fill((100, 100, 100))
            PMOVE.disable_edit()

            self.fill_leds()
            # Flip the display
            pygame.display.flip()
            frames += 1

        # Done! Time to quit.
        pygame.quit()
        end_time = timer()
        duration = end_time - start_time
        fps = frames / duration
        print(f'average FPS = {fps}')


if __name__ == '__main__':
    game = Emulator()
    game.run()